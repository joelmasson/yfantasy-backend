...
app.post("/api/yahoo/:resource/:subresource", authMiddleware, function (req, res) {
    ...
    // console.log(yf[resource][subresource], args);
    // console.profile(`${resource}-${subresource}`);
    yf[resource][subresource].apply(yf[resource], args).then(data => {
        res.send({ data })
    }).catch(err => {
        console.log(err)
    })

    // let data = yf[resource][subresource];
    // data.then((data,err) => {
            ...
            let mappedPlayers = players.map(player => {
                return {
                    'updateOne': {
                        ...
                        'update': { '$set': { 'nhl_player_id': player.id, 'name': player.fullName }, $push: {stats:{coverage_type: 'Season', coverage_value: req.params.id, stats:PlayerStatsDefault}} },
                        'upsert': true
                    }
    ...
    if (req.params.resource === 'games'){
        ...
        if (req.body.action === 'fetch'){
            ...
            if (playByPlays.length > 0){
                ...
                playByPlays.forEach(game => {
                    game.events.forEach((event, i) => {
                        ...
                        if (event.type === "ASSIST" && game.events[i-1].type === "ASSIST") {
                            players[event.playerId][game.gamePk].stats.ASSIST_2 = players[event.playerId][game.gamePk].stats.ASSIST_2 + 1
                        } else {
                            players[event.playerId][game.gamePk].stats[event.type] = players[event.playerId][game.gamePk].stats[event.type] + 1
                            if (event.teamStrength - event.opposingStrength === 1) { // 5 on 4 pp
                                players[event.playerId][game.gamePk].stats['5_ON_4_' + event.type] = players[event.playerId][game.gamePk].stats['5_ON_4_' + event.type] + 1
                            } else if (event.teamStrength - event.opposingStrength === 2) { // 5 on 3 pp
                                players[event.playerId][game.gamePk].stats['5_ON_3_' + event.type] = players[event.playerId][game.gamePk].stats['5_ON_3_' + event.type] + 1
                            } else if (event.opposingStrength - event.teamStrength === 1) { // 4 on 5 pk
                                players[event.playerId][game.gamePk].stats['4_ON_5_' + event.type] = players[event.playerId][game.gamePk].stats['4_ON_5_' + event.type] + 1
                            } else if (event.opposingStrength - event.teamStrength === 2) { // 3 on 5 pk
                                players[event.playerId][game.gamePk].stats['3_ON_5_' + event.type] = players[event.playerId][game.gamePk].stats['3_ON_5_' + event.type] + 1
                            }
                        }
                    })
                })
                ...
                Player.collection.bulkWrite(mappedPlayers.concat(mappedSeasonUpdate)).then(function(data){
                    console.log('P', data)
                    res.send(data)
                }).catch(function(err){
                    console.log('err', err)
